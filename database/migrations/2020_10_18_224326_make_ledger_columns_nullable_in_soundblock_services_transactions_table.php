<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class MakeLedgerColumnsNullableInSoundblockServicesTransactionsTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::table("soundblock_services_transactions", function (Blueprint $table) {
            $table->unsignedBigInteger("ledger_id")->nullable()->change();
            $table->uuid("ledger_uuid")->nullable()->change();
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::table("soundblock_services_transactions", function (Blueprint $table) {
            $table->unsignedBigInteger("ledger_id")->nullable(false)->change();
            $table->uuid("ledger_uuid")->nullable(false)->change();
        });
    }
}
