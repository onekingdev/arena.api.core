...
class QldbCreateSoundblockActionsTable extends Migration
{
    /**
     * @var Ledger
     */
    ...
    {
        try{
            $this->ledgerContract->createTable("soundblock_actions");
    ...
    {
        try{
            $this->ledgerContract->deleteTable("soundblock_actions");
        } catch(LedgerMicroserviceException $exception) {
            Disaster::handleDisaster($exception);
        } catch(\Exception $exception){
            dump($exception->getMessage());
