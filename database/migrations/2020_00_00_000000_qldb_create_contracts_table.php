...
class QldbCreateContractsTable extends Migration
{
    /**
     * @var Ledger
     */
    ...
    {
        try{
            $this->ledgerContract->createTable("soundblock_contracts");
    ...
    {
        try{
            $this->ledgerContract->deleteTable("soundblock_contracts");
        } catch(LedgerMicroserviceException $exception) {
            Disaster::handleDisaster($exception);
        } catch(\Exception $exception){
            dump($exception->getMessage());
