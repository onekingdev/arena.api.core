...
class CreateApparelScrapingRelatedProducts extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create("apparel_scraping_related_products", function (Blueprint $table) {
            $table->bigIncrements("row_id");
            $table->uuid("row_uuid");

            $table->string("product_ascolour_id", 5)->index("idx_product-ascolour-id");
            $table->unsignedBigInteger("product_id")->index("idx_product-id");
            $table->uuid("product_uuid")->index("idx_product-uuid");

            $table->string("related_ascolour_id", 5)->index("idx_related-ascolour-id");
    ...
    {
        Schema::dropIfExists("apparel_scraping_related_products");
    }
}
